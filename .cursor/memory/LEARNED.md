...
### 2026-07-25（デザイン刷新・Claude作業）
- **モバイルアプリ風デザインへ全面刷新**（コーラル系→オレンジ系 `#FF6A00`、フィードを1カラム化、カードを「写真+投稿者名オーバーレイ→タイトル/場所/一言/タグ→いいね・コメント・共有の3ボタン行」構成に）。commit `752f2f5`、push・本番反映済み。
- **事故と教訓**: 直前にCursorが同じ刷新に着手していたが、**CSSの `:root`／レイアウトだけ書き換えてHTML/JS側のマークアップ（`cardHTML()` 等）を追従させないまま**作業が止まっていた（新クラスがCSSにしか存在せずHTML側0件ヒット＝見た目が崩れる状態）。ユーザー確認のうえその未コミット変更は破棄し、ClaudeがCSSとHTML/JSを一体で書き直した。**教訓: デザイン刷新は`<style>`だけ変更して終わりにせず、対応するレンダリング関数（`cardHTML()`/`postDetailHTML()`等）を同じコミット内で必ず追従させる。**
- **壊さなかったもの（意図的に維持）**: ヒーローの `#search`＋`runAssistant()`（自然文検索アシスタント、進捗メモに明記の主力検索UI）は削除・非表示にしていない。イベント委譲対象のクラス名・data属性（`.card` `.like` `.cmt-btn` `.share-btn` `.author-link` `data-cat` `data-id` 等）も無変更。カテゴリフィルタ（`#filters .chip[data-cat]`）は新規追加せず既存を横スクロールpillにrestyleして流用（重複UIを作らない）。
- カード内の「投稿者名」表示位置は、写真がある投稿は写真上に半透明オーバーレイ、写真なし投稿は `card-top` 内にインラインでフォールバック表示（決定済み・変更する場合はユーザー確認）。
...
- ホーム一覧: 同一店舗（住所／座標／店名＋県）の投稿を横スクロールレール表示。場所系カテゴリのみ。単独・他カテゴリは従来グリッドのまま。
- Google Search Console: ドメイン所有権確認済み・sitemap 送信済みを AGENTS に明記。
