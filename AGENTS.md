...
- **プロフィールは全画面ページ** `#pvPage`（`openProfileView()`/`closeProfileView()`）：投稿カードの著者名/アバターから遷移。z-index 200。URL は `?u=<user_id>`。
  ...
  - 構成: 80pxアイコン・名前・@ID・**ベル通知→フォロー**の順・実績4枠（アイコン色分け＋ラベル横並び＋数字/単位）・自己紹介全文＋信頼バッジ・フィルター・**3列投稿カード**（縦長3:4・角丸・画像オーバーレイ：左上カテゴリ／右上保存／左下タイトル＋場所／右下いいね数）・もっと見る（9件ずつ）。**位置情報行なし。下部CTAなし。**
