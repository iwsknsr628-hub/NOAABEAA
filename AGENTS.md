...
5. **UI/デザイン実装は基本的に Claude（Cowork）が担当する**（2026-07-25、ユーザーと合意）。理由: Cursorが会話の裏で並行して同じ箇所を編集し、未完成のまま放置される事故が2回発生した（CSSだけ書いて停止／「いいね」と「保存」を混同したまま停止。詳細は `.cursor/memory/LEARNED.md` の2026-07-25項目）。**Cursorは、ユーザーから明示的に指示されない限り、UI/デザインの自発的な変更に着手しない**こと。着手前に必ず `git status`（他エージェントの未コミット変更が無いか）を確認する。
...
- **プロフィールは全画面ページ** `#pvPage`（`openProfileView()`/`closeProfileView()`）：投稿カードの著者名/アバターから遷移。z-index 200。URL は `?u=<user_id>`。
  ...
  - 構成: 80pxアイコン・名前・@ID・フォローボタン・実績4枠（アイコン色分け＋ラベル横並び＋数字/単位）・自己紹介全文＋信頼バッジ・フィルター・**3列投稿カード**（縦長3:4・角丸・画像オーバーレイ：左上カテゴリ／右上保存／左下タイトル＋場所／右下いいね数）・もっと見る（9件ずつ）。**位置情報行なし。下部CTAなし。**
  - ヘッダー右上: **通知ベル（他ユーザー閲覧時・要ログイン）→ 共有 → ⋯**。
  - 投稿モーダル `#modalBg` は z-index 220（プロフィール全画面 200 より上）。ピン地図は 235。
...
- **デザイン**: 現行1系統のみ（コーラル／ネイビー／Zenフォント／ロゴ「なんしよ。」／すっきりヒーロー）。旧 classic テーマ・切替UIは廃止。
  - ヒーロー全面写真: `/icons/hero-field.png`（左右フルブリード）。旧イラスト `thinking-girl-line.png` は廃止。CTA: **現在地で探す** / **全国で探す** / **気分で探す**（`searchNearNow` / `searchNationwide` / `openMoodPage`）。
  - **気分で探す** `#moodPage`：全画面。丸カテゴリ（デート／ひとり／がっつり／癒し／美容）＋並び（おすすめ／新着／近い）＋画像オーバーレイカード（タイトル・一言・保存・場所・いいね）。
