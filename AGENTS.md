...
- **プロフィールは全画面ページ** `#pvPage`（`openProfileView()`/`closeProfileView()`）：投稿カードの著者名/アバターから遷移。z-index 200。URL は `?u=<user_id>`。
  - 構成: 戻る/共有/⋯ · 64pxアイコン+名前+一言(30字)+エリア+フォロー · 実績4（投稿/保存/いいね/スポット）· 信頼「実体験のみ · PRなし」· フィルター（すべて/グルメ/遊び/美容/まったり）· 2列カード（♡保存数）· 下部固定CTA（保存一覧/マップで見る/まとめを見る）。
  - エリアはプロフィール住所欄が無いため投稿から算出した「○○周辺」。マップは Leaflet（`openPvMap`）。フォロー/通知/SNSリンク/@IDは既存機能を維持。
  - 色: `#111` / `#FFF` / アクセント `#FF6A00`（フォロー・マップCTA）。
